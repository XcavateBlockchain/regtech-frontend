...
| Topic | File |
|---|---|
| Phantom provider config | `providers/solana-provider.tsx` |
| Wallet façade hook | `hooks/use-wallet-kit.ts` |
| Phantom auth message + verify | `lib/phantom-auth-message.ts`, `lib/phantom-auth.ts` |
| Phantom signature normalization | `lib/phantom-signature.ts` |
| Phantom → kit signer adapter | `lib/solana/phantom-wallet-session.ts` |
| Swig create + delegate sign | `lib/solana/admin.ts` |
| Hand-written PDAs | `lib/solana/pda.ts` |
| Codama config + output | `codama.json`, `generated/reg_tech/**` |
| RPC singleton + React action hooks | `hooks/use-contract.ts` |
| Swig account React hook | `hooks/use-swig.ts` |
| Server enroll example | `lib/server/enroll.ts` |
| Owner registration route | `app/api/auth/register/route.ts` |
| Start-attempt route (enroll + start) | `app/api/module/[moduleId]/start/route.ts` |
