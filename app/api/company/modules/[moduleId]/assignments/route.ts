...
export async function GET(
  ...
  try {
    ...
    console.error("[GET /api/company/modules/:moduleId/assignments]", e);
    return NextResponse.json(
...
export async function POST(
  ...
  try {
    ...
    for (const employeeUserId of employeeUserIds) {
      ...
      // Create assignment row first (owner requested assignment).
    ...
    console.error("[POST /api/company/modules/:moduleId/assignments]", e);
    return NextResponse.json(
      { error: e instanceof Error ? e.message : "Internal server error" },
      { status: 500 },
    );
